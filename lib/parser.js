...
function cacheVariant(componentName, props, children, visualSpec) {
  if (!VARIANT_CACHE[componentName]) VARIANT_CACHE[componentName] = [];
...
function parseFigmaNode(node, tokenLookup) {
  ...
  if (type === 'INSTANCE') {
    ...
    return {
      ...
      _inferred_content: inferredText
    };
  }
  ...
  if (type === 'TEXT') parsed.text = node.characters;
...
function createLayoutBlueprint(node, isRoot = false, keepVisuals = false) {
  ...
  if (!isRoot && node.type === 'COMPONENT_INSTANCE') {
    ...
    const blueprint = {
      ...
      props: finalProps,
      visualSpec: { box: { display: node.visualSpec?.box?.display } },
      children: [],
      isBlueprint: true,
